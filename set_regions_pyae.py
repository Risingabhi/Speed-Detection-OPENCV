...
from crop_play import make_mask
...
if __name__ == '__main__':
    ...
    global savePath
    global output_frame
    savePath = args.out_file if args.out_file.endswith(
        ".p") else args.out_file+".p"
    ...
    while video_capture.isOpened():
        ...
        if not success:
            break
        if cnt == 50:  # will use frame no 50 for making mask
            rgb_image = frame[:, :, ::-1]
            output_frame = frame
        cnt += 1
        #saving 15 frmaes from camera
        for i in range(15):                 #modify
            cv2.imwrite('Frames'+str(i)+'.jpg', frame)            #modify
