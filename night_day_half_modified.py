...
global start_time,start_time1,later,later1,starttime,endtime,cy2, maxLoc
...
while True:
    ...
    if score > 200.0: # DAY TIME
        ...
        cv2.line(frame_og,(l1_x1,pt3_pxl),(l1_x2,pt3_pxl),(125,200,127),3)
    ...
    #     break
        ...
        flag2 = True
        list_speed=[]
        #============================================================================PIXEL MAPPING OR LINE ON NEW ROI 
        h,w,_ =frame.shape
        x=0
        roi_ = frame[int(h/2): int(h),int(x):int(x+w)]
        new_x = l1_x1
        relative_start_y = int(h)-int(l1_y1)
        new__start_y = int(h/2)-relative_start_y
        cv2.circle(roi_,(new_x,new__start_y),4,(0,0,255),-1)

        relative_mid_y = int(h)-int(l2_y1)
        new_mid_y = int(h/2)-relative_mid_y
        cv2.circle(roi_,(new_x,new_mid_y),4,(0,0,255),-1)

        relative_end_y = int(h)-int(pt3_pxl)
        new_end_y = int(h/2)- relative_end_y
        cv2.circle(roi_,(new_x,new_end_y),4,(0,0,255),-1)
        print(roi_.shape)
        ...
        gray = cv2.cvtColor(roi_, cv2.COLOR_BGR2GRAY)

        gray = cv2.GaussianBlur(gray, (41, 41), 0)

        (minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray)
        cv2.imshow("half",roi_)
        cv2.waitKey(1)
        h,w=(gray.shape)
        #to find speed
        cx2,cy2 = maxLoc
        print("cx",cx2)
        print("cy2",cy2)
        print("new_start_y",new__start_y)
        print("new_mid_y",new_mid_y)
        print("new_end_y",new_end_y)
    elif cy2 > new__start_y:
        if flag2 is True and cy2 < new_mid_y:
            starttime= datetime.datetime.now()
            flag2 = False
        elif cy2> new_mid_y and cy2< new_end_y:
            endtime = datetime.datetime.now()
            timedelta = (endtime - starttime).total_seconds()
            frame_crossed3 = timedelta*framespersecond
            speed1 = (distance/frame_crossed3)*framespersecond*3.6         #####COSINE CORRECTION############## '''takes an angle of camera'''
            Angle = math.radians(Angle)
            Angle = math.cos(Angle)
            ...
            print("frame_night",frame_crossed3)              
            ...
            if int(avg_speed) > speed_limit and cy2 > lasttrack:
                ...
                lane_1_1.append(roi)
                #send(roi)
		cv2.circle(roi_, maxLoc, 10, (255, 0, 255), -1)
		cv2.line(roi_, (l1_x1, new__start_y), (l1_x2, new__start_y), (0, 255, 0), 1)
		cv2.line(roi_, (l2_x1, new_mid_y), (l2_x2, new_mid_y), (0, 0, 255), 1)
		cv2.line(roi_, (l1_x1, int((new_end_y))), (l1_x2, int((new_end_y))),(0, 0, 0), 1)
		cv2.imshow("Robust",roi_ )
